...
func (c *DocumentController) Read() {
	...
	tree, err := models.NewDocument().CreateDocumentTreeForHtml(bookResult.BookId, doc.DocumentId, c.Member.MemberId)
...
func promptUserToLogIn(c *DocumentController) {
	...
	if c.IsAjax() {
		...
		redirecturl := loginUrl + "?appId=" + appid + "&url=" + url.PathEscape(sysUrl)
